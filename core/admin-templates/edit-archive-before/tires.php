...
<div class="admin-section general-content">
	<form class="form-style-basic js-remove-empty-on-submit" method="get" action="<?php echo get_admin_archive_link( 'tires' ); ?>">
		<?php
		...
		echo '<p>Using the items below, you can look for part numbers that were uploaded in a particular import and delete them.</p>';
		echo '<br>';

		echo '<div class="form-items inline">';

		// spits out a <select> containing the unique values of a column
		$stuff = function( $col ){
			echo get_form_select_from_unique_column_values( 'tires', $col, get_user_input_singular_value( $_GET, $col ) );
		};
		...
		$stuff( 'import_name' );
		$stuff( 'import_date' );
		echo get_form_select_from_unique_column_values( 'tires', 'import_date', get_user_input_singular_value( $_GET, $import_date_not ), $import_date_not );
...
//		$stuff( 'sold_in_us' );
		...
		echo '<br>';
		echo '<p><a href="' . get_admin_archive_link( 'tires' ) . '">Reset</a></p>';
		echo '<br>';
