...
Class Wheel_Size_Api {
    ...
    public static $url = 'https://api.wheel-size.com/v2';
...
//			);
        ...
        // V2 API returns count and data separately
...
//    ];
    ...
    $r = array_filter( is_array( $r ) ? $r : [], function( $trim ){
        ...
        $markets = gp_force_array( @$trim['regions'] );
        ...
        return count(array_intersect(Wheel_Size_Api::MARKETS_ALLOWED, $markets)) >= 1;
    });
    ...
    $r = array_map( function( $trim ) use( $add_options_string_to_name ){
        ...
        return [
            ...
            'trim_levels' => is_array( @$trim['trim_levels'] ) ? $trim['trim_levels'] : []
        ];
    ...
    foreach ( $r as $trim ) {
        ...
        $ret[$slug] = $add_options_string_to_name( $ret[$slug], $trim['trim_levels']);
    }
...
function get_fitment_data( $make, $model, $year, $trim ) {
    ...
    // this is a market priority system. We check for results in each market until a result is found, and then
    ...
    if ( $r && is_array( $r ) ) {
        foreach ( $r as $result ) {
            $market = gp_if_set( $result, 'regions' );

            $found = false;
            if ( $markets ) {
                foreach ( $markets as $ms ) {
                    if ( in_array($ms, $market) ) {
                        $found = true;
                        $fd_single = $result;
                        break;
                    }
    ...
    if ( $fd_single ) {
        ...
        $market = gp_if_set( $fd_single, 'region' );
